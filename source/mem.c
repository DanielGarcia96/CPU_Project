...
char Data[4096];
//void mem(char addr_16bits[], char data[]);
...
//   char memoryArray[16][4096] = {};
   ...
   if( addr_16bits[0]>='A' && addr_16bits[0]<='F' ) chipSel = addr_16bits[0]-(int)('A')+10;
   if( addr_16bits[0]>='a' && addr_16bits[0]<='f' ) chipSel = addr_16bits[0]-(int)('a')+10;
   if( addr_16bits[0]>='0' && addr_16bits[0]<='9' ) chipSel = addr_16bits[0]-(int)('0');
   ...
   for(int j = 1; j < sizeof(addr_16bits); j++)
   {
      ...
      if( addr_16bits[j] >='A' && addr_16bits[j]<='F' ) number += addr_16bits[j]-(int)('A')+10;
      if( addr_16bits[j]>='a' && addr_16bits[j]<='f' ) number += addr_16bits[j]-(int)('a')+10;
      if( addr_16bits[j]>='0' && addr_16bits[j]<='9' ) number += addr_16bits[j]-(int)('0');
   }
